...
CONFIG_SCHEMA = {
    ...
    "max_split": {"env": "SIMMER_MERT_MAX_SPLIT", "default": 0.95, "type": float},
...
MAX_SPLIT = _config.get("max_split", 0.95)  # Default to 95% if not set
...
def run_mert_strategy(dry_run=True, positions_only=False, show_config=False,
    ...
    for market in expiring_markets:
        ...
        # Get both prices to determine the favorite correctly
        # Priority: yesPrice/noPrice > current_price_yes/current_price_no > current_probability
        cp = market.get("current_probability", 0.5)
        if cp is None:
            cp = 0.5
        
        yes_price = float(market.get("yesPrice") or market.get("current_price_yes") or cp)
        no_price = float(market.get("noPrice") or market.get("current_price_no") or (1 - cp))
        
        # Handle case where prices might be None or 0.5/0.5
        ...
        print(f"     Split: YES {yes_price:.0%} / NO {no_price:.0%}")

        # Determine the favorite (the side with higher probability)
        ...
        # Check split threshold - favorite must be between MIN_SPLIT and MAX_SPLIT
        if favorite_price < MIN_SPLIT:
            print(f"     Skip: split too narrow ({yes_price:.0%}/{no_price:.0%}, need {MIN_SPLIT:.0%}+)")
            continue
        if favorite_price > MAX_SPLIT:
            print(f"     Skip: split too wide ({yes_price:.0%}/{no_price:.0%}, need {MAX_SPLIT:.0%} or less)")
            continue

        strong_split_count += 1

        # Buy the favorite (higher probability side)
        side = favorite
        side_price = favorite_price
